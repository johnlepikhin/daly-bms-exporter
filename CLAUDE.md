...
- **Публикуемый набор пиров выбирается по серийнику, а не по величине
  расхождения** (`peer_max_published`, по умолчанию 3). Выбор «худшего пира»
  заставлял метку `peer` скакать между скрейпами и рвал ряды на дашборде в
  осколки по одной-две точки. Решение о заморозке при этом считает максимум по
  **всем** подходящим пирам, включая не попавшие под потолок, — иначе потолок
  кардинальности прятал бы вето.
